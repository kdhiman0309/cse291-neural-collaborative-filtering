...
class Dataset(object):
    ...
    def gen_train_data(self):
        t1 = time()
        user_input, item1_input,item2_input, labels = [],[],[],[]
        ...
        if self.num_threads>1:
            ...
            for index in self.trainData.index:
                ...
                for _i in range(len(negatives)):
                    ...
                    if c%2 == 0:
                        user_input.append(u)
                        item1_input.append(i)
                        item2_input.append(neg_item_ID)
                        labels.append(1)
                    else:
                        user_input.append(u)
                        item1_input.append(neg_item_ID)
                        item2_input.append(i)
                        labels.append(0)
                    c = c+1

        user_input, item1_input, item2_input, labels = shuffle(user_input, item1_input, item2_input, labels)
        m = ModelData()
        m.userids = np.array(user_input)
        m.item1ids = np.array(item1_input)
        m.item2ids = np.array(item2_input)
