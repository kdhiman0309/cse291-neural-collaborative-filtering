...
def evaluate_model_inference(model, testData, dataset, K, num_thread):
    """
    Evaluate the performance (Hit_Ratio, NDCG) of top-K recommendation
    Return: score of each test rating.
    """
    ...
    if(num_thread > 1): # Multi-thread
        pool = multiprocessing.Pool(processes=num_thread)
        res = pool.map(eval_one_rating_inference_mp, testData.index)
        pool.close()
        pool.join()
        hits = [r[0] for r in res]
        ndcgs = [r[1] for r in res]
    ...
    # Single thread
    for index,row in testData.iterrows():
        (hr,ndcg, auc) = eval_one_rating_inference(row, model, K, dataset)
        hits.append(hr)
        ndcgs.append(ndcg)    
        aucs.append(auc)
    return (hits, ndcgs, aucs)

def eval_one_rating_inference_mp(idx):
     return eval_one_rating(_testData.loc[idx],_model,_K)

def eval_one_rating_inference(row, model, K, dataset):
